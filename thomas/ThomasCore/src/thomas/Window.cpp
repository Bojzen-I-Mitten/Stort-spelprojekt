...
namespace thomas 
{
	...
	{
		WaitForSingleObjectEx(m_frameLatancyWaitableObject, 1000, true);
	}
	...
	void Window::Bind()
	{
		utils::D3D::Instance()->GetDeviceContext()->OMSetRenderTargets(1, &m_dxBuffers.RTV, m_dxBuffers.depthStencilView);
	...
	bool Window::InitDxBuffers()
	{
		...
		utils::D3D::Instance()->CreateRenderTarget(m_swapChain, m_dxBuffers.buffer, m_dxBuffers.RTV, true, m_dxBuffers.SRV);
		utils::D3D::Instance()->CreateDepthStencilView(m_dxBuffers.depthStencilView, m_dxBuffers.depthStencilViewReadOnly, m_dxBuffers.buffer, true);
