...
namespace thomas
{
	namespace resource
	{
		...
		{
			//Bind();
			utils::D3D::Instance()->GetDeviceContextDeferred()->Dispatch(threadGroupX, threadGroupY, threadGroupZ);
		}

		void ComputeShader::DispatchIndirect(ID3D11Buffer* indirectBuffer, unsigned alignedByteOffsetForArgs)
		{
			utils::D3D::Instance()->GetDeviceContextDeferred()->DispatchIndirect(indirectBuffer, alignedByteOffsetForArgs);
		}

		void ComputeShader::UnbindAllUAVs()
		{
			utils::D3D::Instance()->GetDeviceContextDeferred()->CSSetUnorderedAccessViews(0, 8, s_nullUAVs, nullptr);
		}
		...
		void ComputeShader::UnbindOneUAV(unsigned startIndex)
		{
			...
			utils::D3D::Instance()->GetDeviceContextDeferred()->CSSetUnorderedAccessViews(startIndex, 1, s_nullUAV, nullptr);
		}

		void ComputeShader::UnbindAllSRVs()
		{
			utils::D3D::Instance()->GetDeviceContextDeferred()->CSSetShaderResources(0, 8, s_nullSRVs);
		}
		...
		void ComputeShader::UnbindOneSRV(unsigned startIndex)
		{
			...
			utils::D3D::Instance()->GetDeviceContextDeferred()->CSSetShaderResources(startIndex, 1, s_nullSRV);
		}
