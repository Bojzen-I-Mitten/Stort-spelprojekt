...
namespace thomas
{
	namespace resource
	{
		...
		void Font::DrawGUIText(SpriteBatch* spritebatch, const std::string& text, const Vector2& position,
							   const Vector2& scale, const Vector2& origin, const Vector4& color, float rotation)
		{
			m_font->DrawString(spritebatch, utility::ToWChar(text).c_str(), position, color, rotation, origin * GetTextSize(text), scale);
		}
