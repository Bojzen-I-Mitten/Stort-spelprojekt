...
namespace thomas
{
	namespace graphics
	{
		...
		void ParticleSystem::Initialize(unsigned maxNrOfParticles, BLEND_STATE blendState)
		{
			...
			//utils::D3D::Instance()->GetDeviceContextDeffered()->CopyStructureCount(m_bufferCounters->GetBuffer(), 4, m_bufferAliveListPing->GetUAV());
		...
		void ParticleSystem::SpawnParticles()
		{
			...
			utils::D3D::Instance()->GetDeviceContextDeffered()->CSSetConstantBuffers(7, 1, cBuffer);
		...
		void ParticleSystem::UpdateAliveCount()
		{
			...
			{
				utils::D3D::Instance()->GetDeviceContextDeffered()->CopyStructureCount(m_bufferCounters->GetBuffer(), 4, m_bufferAliveListPong->GetUAV());
			}

			utils::D3D::Instance()->GetDeviceContextDeffered()->CopyStructureCount(m_bufferCounters->GetBuffer(), 0, m_bufferDeadList->GetUAV());
		...
		void ParticleSystem::DrawParticles()
		{
			...
			utils::D3D::Instance()->GetDeviceContextDeffered()->DrawInstancedIndirect(m_bufferIndirectArgs->GetBuffer(), 12);

			ID3D11ShaderResourceView* const s_nullSRV[8] = { NULL };
			utils::D3D::Instance()->GetDeviceContextDeffered()->VSSetShaderResources(0, 8, s_nullSRV);
