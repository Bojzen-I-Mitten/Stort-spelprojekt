...
NK_API void nk_d3d11_render(enum nk_anti_aliasing);
...
NK_API void nk_d3d11_render(enum nk_anti_aliasing AA)
{
    ...
    g_context->IASetInputLayout(d3d11.input_layout);
	g_context->IASetVertexBuffers(0, 1, &d3d11.vertex_buffer, &stride, &offset);
    g_context->IASetIndexBuffer(d3d11.index_buffer, DXGI_FORMAT_R16_UINT, 0);
	g_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    g_context->VSSetShader(d3d11.vertex_shader, NULL, 0);
    g_context->VSSetConstantBuffers(0, 1, &d3d11.const_buffer);

    g_context->PSSetShader(d3d11.pixel_shader, NULL, 0);
    g_context->PSSetSamplers(0, 1, &d3d11.sampler_state);

    g_context->OMSetBlendState(d3d11.blend_state, blend_factor, 0xffffffff);
    g_context->RSSetState(d3d11.rasterizer_state);
    g_context->RSSetViewports(1, &d3d11.viewport);
    ...
    hr = g_context->Map((ID3D11Resource *)d3d11.vertex_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &vertices);
    NK_ASSERT(SUCCEEDED(hr));
    hr = g_context->Map((ID3D11Resource *)d3d11.index_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &indices);
    NK_ASSERT(SUCCEEDED(hr));
    ...
    g_context->Unmap((ID3D11Resource *)d3d11.vertex_buffer, 0);
    g_context->Unmap((ID3D11Resource *)d3d11.index_buffer, 0);
    ...
    nk_draw_foreach(cmd, &d3d11.ctx, &d3d11.cmds)
    {
        ...
        scissor.bottom = (LONG)(cmd->clip_rect.y + cmd->clip_rect.h);
         ...
         g_context->PSSetShaderResources(0, 1, &texture_view);
         g_context->RSSetScissorRects(1, &scissor);
         g_context->DrawIndexed((UINT)cmd->elem_count, offset, 0);
...
NK_API struct nk_context* nk_d3d11_init(ID3D11Device *device, ID3D11DeviceContext* context, int width, int height, unsigned int max_vertex_buffer, unsigned int max_index_buffer)
{
    ...
    hr = g_device->CreateRasterizerState(&desc, &d3d11.rasterizer_state);
    NK_ASSERT(SUCCEEDED(hr));}

    /* vertex shader */
    {hr = g_device->CreateVertexShader(nk_d3d11_vertex_shader, sizeof(nk_d3d11_vertex_shader), NULL, &d3d11.vertex_shader);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    hr = g_device->CreateInputLayout(layout, ARRAYSIZE(layout), nk_d3d11_vertex_shader, sizeof(nk_d3d11_vertex_shader), &d3d11.input_layout);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    hr = g_device->CreateBuffer(&desc, &data, &d3d11.const_buffer);
    NK_ASSERT(SUCCEEDED(hr));}}

    /* pixel shader */
    {hr = g_device->CreatePixelShader(nk_d3d11_pixel_shader, sizeof(nk_d3d11_pixel_shader), NULL, &d3d11.pixel_shader);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    hr = g_device->CreateBlendState(&desc, &d3d11.blend_state);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    desc.MiscFlags = 0;
    hr = g_device->CreateBuffer(&desc, NULL, &d3d11.vertex_buffer);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = g_device->CreateBuffer(&desc, NULL, &d3d11.index_buffer);
    NK_ASSERT(SUCCEEDED(hr));}
    ...
    desc.MinLOD = 0.0f;
    desc.MaxLOD = FLT_MAX;
    hr = g_device->CreateSamplerState(&desc, &d3d11.sampler_state);
    NK_ASSERT(SUCCEEDED(hr));}
...
NK_API void nk_d3d11_font_stash_end(void)
{
    ...
    data.SysMemSlicePitch = 0;
    hr = g_device->CreateTexture2D(&desc, &data, &font_texture);
    assert(SUCCEEDED(hr));}
    ...
    hr = g_device->CreateShaderResourceView((ID3D11Resource *)font_texture, &srv, &d3d11.font_texture_view);
    assert(SUCCEEDED(hr));}
