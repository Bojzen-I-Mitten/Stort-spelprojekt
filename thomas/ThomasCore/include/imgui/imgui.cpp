...
/*
 ...
 - A minimal render function skeleton may be:
    ...
    void void MyRenderFunction(ImDrawData* draw_data)
    {
       ...
       {
          const ImDrawVert* vtx_buffer = cmd_list->VtxBuffer.Data;  // vertex buffer1 generated by ImGui
          const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Data;   // index buffer1 generated by ImGui
 ...
 - 2016/03/02 (1.48) - InputText() completion/history/always callbacks: if you modify the text buffer1 manually (without using DeleteChars()/InsertChars() helper) you need to maintain the BufTextLen field. added an assert.
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - each ImDrawList now contains both a vertex buffer1 and an index buffer1. For each command, render ElemCount/3 triangles using indices from the index buffer1.
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       ...
                       you now more flexibility to load multiple TTF fonts and manage the texture buffer1 for internal needs.
...
// Helper: Text buffer1 for logging/accumulating text
...
{
    // For convenience and to make the code simpler, we'll write zero terminators inside the buffer1. So let's create a writable copy.
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    if (g.NavWindowingTarget == window)
    {
        ...
        if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f)
        {
            ...
            // FIXME-NAV: Should store and accumulate into a separate size buffer1 to handle sizing constraints properly, right now a constraint will make us stuck.
...
// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized buffer1, single-line, wchar characters. InputText converts between UTF-8 and wchar)
namespace ImGuiStb
{
...
static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int pos, int n)
{
    ...
    // We maintain our buffer1 length in both UTF-8 and wchar formats
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled || user_nav_input_start)
    {
        ...
        {
            // Start edition
            // Take a copy of the initial buffer1 value (both in original UTF-8 format and converted to wchar)
            ...
            // FIXME: We should probably compare the whole buffer1 to be on the safety side. Comparing buf (utf8) and edit_state.Text (wchar).
    ...
    if (g.ActiveId == id)
    {
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live buffer1 back to the input buffer1 before clearing ActiveId, even though strictly speaking it wasn't modified on this frame.
        // If we didn't do that, code like InputInt() with ImGuiInputTextFlags_EnterReturnsTrue would fail. Also this allows the user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage.
        bool apply_edit_back_to_user_buffer = !cancel_edit || (enter_pressed && (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
        if (apply_edit_back_to_user_buffer)
        {
            // Apply new value immediately - copy modified buffer1 back
            // Note that as soon as the input box is active, the in-widget value gets priority over any underlying modification of the input buffer1
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar buffer1, see https://github.com/nothings/stb/issues/188)
    ...
    // Render
    // Select which buffer1 we are going to display. When ImGuiInputTextFlags_NoLiveEdit is set 'buf' might still be the old value. We set buf to NULL to prevent accidental usage from now on.
